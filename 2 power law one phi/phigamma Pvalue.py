...
def P1(E,n,g,e,a,bk):  # of unit s-1 cm-2
    p=n*1e-18*(((E)/1e5)**-g)
...
#########################
#########################
phi=np.linspace(0,5,num=101)
gamma=np.linspace(2,4,num=101)
gamma2=np.linspace(2,4,num=101)
    ...
    for k in range(0,len(gamma)):
        for a in range(0,len(gamma2)):
            ...
            for b in range(0,len(et2)):
                theory=P1(et2[b],phi[i],gamma[k],et1[b],area[b],background[b])
                chisquared=chi(bins[b],theory)
                p1.append(chisquared)
                theoryw=P1(et2[b],phi[i],gamma2[a],et1[b],area[b],background[b])
