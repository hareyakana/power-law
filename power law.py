...
def P(e):  # of unit s-1 cm-2
    g=2.58
    n=8.5e-18 ##phi best fit from IceCube
    p=n*((e/10e5)**-g)
    return p
...
for x in range(0,7):
    area[2*x]=area[2*x]*2.45/2.

for x in range(0,8):
    area[x*2-1]=area[x*2-1]*2.55/3.
    

theory=P(et1)*time*4*m.pi*area*(et2-et1)+bkmuon+bkatm+atmnu
...
pl.grid()
pl.scatter(et ,bins)
pl.step(et2,theory,label='background+power law')
...
print("p-value")
print(value)
print("mean p", sum(value)/len(value))
